...
word_bank = WordBankManager(language="english")
...
async def check_word_viability(request: Request):
    ...
    try:
        ...
        ascii_data = word_bank.encode_word(data['word'].lower())
        if np.any(np.all(word_bank.full_word_bank == ascii_data, axis=1)):
            return {"valid": True}
        return {"valid": False}
...
async def get_possible_words(request: Request):
    ...
    try:
        ...
        word_bank.reset()  # Reset the word bank before processing

        # if 0 <= data['currentRow'] < len(data['feedback']):
        #     data['feedback'][data['currentRow']] = ["green" if c != "error" else c
        #                                             for c in data['feedback'][data['currentRow']]
        #                                             ]
        ...
        for guess, feedback in zip(data['words'], data['feedback']):
            guess = word_bank.encode_word(guess.lower())
            word_bank.cull(guess, feedback)

        result = {'possible_words_left': str(len(word_bank.possible_word_bank))}
        return result
...
@app.post("/wordle")
async def submit_wordle_guess(request: Request):
    # Parse and process the incoming data
    try:
        data = await request.json()  # Try to parse the JSON request body

        # Reset the game state before finding next guess
        wordle_guesser.reset_game()
...
async def random_viable_guess():
    ...
    try:
        ...
        if word_bank.possible_word_bank.size > 0:
            random_possible_guesses_row = word_bank.possible_word_bank[
                np.random.randint(word_bank.possible_word_bank.shape[0])
            ]
            random_possible_guess = word_bank.decode_word(random_possible_guesses_row)
...
async def random_word():
    ...
    try:
        random_possible_guess = "No Viable Options"
        if word_bank.possible_word_bank.size > 0:
            random_possible_guesses_row = word_bank.full_word_bank[
                np.random.randint(word_bank.full_word_bank.shape[0])
            ]
            random_possible_guess = word_bank.decode_word(random_possible_guesses_row)
